...
одновременного обновления major-версии БД. Текущий сервер уже использует
MariaDB 10.11.18, а контейнер — MariaDB 10.11.19 той же LTS-ветки.
...
MariaDB (service name остаётся `mysql`):
...
6. Переключите Finntrail virtual host в FASTPANEL/Nginx на Docker upstream.
...
MariaDB и массовый рефакторинг приложения: эти изменения лучше разделить, чтобы
